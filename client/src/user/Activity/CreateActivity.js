...
const CreateActivity = () => {
    ...
    return (
        <div id="create_activity">
                    ...
                    <select className="tdActCon" onChange={(e) => setPartnerId(e.target.value)}>
                        ...
                        {partners.length!==0 ?
                            partners.map((partner, index) => {
                                return (
                                    <option key={index} value={partner.partnerId}>{partner.partnerName}({checkPartnerDetail(partner.partnerDetail)})</option>
                                    )
                            })
                        :
            ...
            <img src={Notifications_Flatline} height="250" width="250" id="bottom_svg"></img>
        </div>
    );
};
